  ...
  return(
    <View style={styles.scene}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.column}>
        <Text style={[{fontSize:40, fontWeight:'bold'}]}>{valueConverted}{' '}</Text>
        </View>
      </View> 
    </View>
  )
...
export default function App() {
  ...
  return (
    <View style={[styles.scene, {marginTop: Constants.statusBarHeight}]}>
      <Text style={styles.title}>UNIT CONVERTOR APP</Text>
      <TabView navigationState={{index, routes}}
        renderScene={renderScene}
        onIndexChange = {setIndex}
        initialLayout={{width: Dimensions.get('window').width }}
        renderTabBar={(props) => (
          <TabBar {... props} scrollEnabled tabStyle={{width:'auto'}} indicatorStyle={{ backgroundColor:'pink'}} style={{backgroundColor:mainColor}}/>
        )}
      ></TabView>
    </View>
  );
